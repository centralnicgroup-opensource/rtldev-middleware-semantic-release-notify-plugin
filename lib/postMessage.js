...
async function retryFn(message, logger, { teamsWebhook, githubToken }) {
    ...
    await publishNotification(message, logger, teamsWebhook, githubToken);
};
retryFn.counter = 1;
...
export default async function postMessage(message, logger, { teamsWebhook, githubToken }) {
    ...
    try {
        ...
        isSuccess = response.ok && bodyText === '2';
    } catch (e) {
        // Handle fetch errors
    ...
    if (!isSuccess) {
        await retryFn(message, logger, teamsWebhook, githubToken);
