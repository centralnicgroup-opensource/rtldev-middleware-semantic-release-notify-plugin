...
const publishNotification = async (payload) => {
    const webhookUrl = process.env?.TEAMS_NOTIFICATION_URI;
    try {
        // Convert the JSON payload to a string and escape single quotes
        const escapedPayload = JSON.stringify(payload).replace(/'/g, "'\\''");
        const res = execSync(`curl -sS -X POST -H 'Content-Type: application/json' -d '${escapedPayload}' '${webhookUrl}'`, { encoding: "utf-8" });
        if (res.toString() === '1') {
            console.log(
...
export { updateValues, isObjectMatch, findAndReplaceNestedKey, readPackageJson, retryFn, fetchPullRequestInfo, publishNotification };